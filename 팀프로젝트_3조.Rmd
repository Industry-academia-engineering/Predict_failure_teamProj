...
model_rf <- randomForest(Target ~ Air_temperature_K + Process_temperature_K + 
                         ...
                         data = datTrain, importance = TRUE)
# 모델 요약
print(model_rf)

# 모델 예측
predictions <- predict(model_rf, newdata = datTest)

# 혼동 행렬 생성
confMat <- confusionMatrix(predictions, datTest$Target)
print(confMat)
...
rf_tuned <- train(Target ~ Air_temperature_K + Process_temperature_K + 
                  Rotational_speed_rpm + Torque_Nm + Tool_wear_min + Type,
                  data = datTrain, method = "rf", trControl = control, tuneGrid = tuneGrid)

# 최적 모델 확인
print(rf_tuned)

# 최적 모델로 예측 및 평가
predictions_tuned <- predict(rf_tuned, newdata = datTest)
confMat_tuned <- confusionMatrix(predictions_tuned, datTest$Target)
print(confMat_tuned)
